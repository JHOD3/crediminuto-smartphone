...
;(function($, window, document, undefined) {
  ...
  $.fn.imageuploadify = function(opts) {
    ...
    //   }
      ...
      let button = dragbox.find(".imageuploadify-images-list-btn a");
      ...
      const readingFile = (id, file) => {
        ...
        let container = $(`<div class='imageuploadify-container mb-0'>
          ...
          <div class='imageuploadify-details'>
            <span class="d-flex text-muted" style="width: 160px;"><i class="fas fa-check text-success mr-2" style="font-size: 12px;margin-top: 2px;"></i> ${file.name}</span>
          </div>
        </div>`);
        ...
        if (file.type && file.type.search(/image/) != -1) {
          ...
          fReader.onloadend = function (e) {
            ...
            $('#well').css({'padding-top':' 20px'});
            if ($(self).attr("multiple")) {
                $('#texto-agregar').text('Añade otro archivo');
               
                $('#imageuploadify-images-list-btn').removeClass('m-auto').addClass('ml-3');
                $('#imageuploadify-images-list-btn').css('margin','20px');
                
                $('#imageuploadify-images-list-btn a').removeClass('btn-success');
                $('.btn-floating i').css({
                'font-size':'10px',
                'color':'rgb(21, 234, 132) !important',
                'margin-left': '-1px',
                'color':'rgb(21, 234, 132)'
                });
            ...
            if (!$(self).attr("multiple")) {
                $('#imageuploadify-images-list-btn').removeClass('d-flex').addClass('d-none'); 
            }
            $('.btn-continuar').prop('disabled', false);        // enables button
            $('.btn-continuar').addClass('btn-danger').removeClass('btn-light disabled');
          ...
          for (let index = 0; totalFiles.length > index; ++index) {
            if (totalFiles[index].id === id) {
              if (index < 1) {
                ...
                if (!$(self).attr("multiple")) {
                    $('#imageuploadify-images-list-btn').removeClass('d-none').addClass('d-flex');
                }                
              }else{
                $('.btn-continuar').prop('disabled', false);        // enables button
                $('.btn-continuar').addClass('btn-danger').removeClass('btn-light disabled');
